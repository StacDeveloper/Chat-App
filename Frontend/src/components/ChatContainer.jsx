...
import assets, { messagesDummyData } from '../../chat-app-assets/chat-app-assets/assets'
...
const ChatContainer = () => {
    ...
    const handleSendingImage = async (e) => {
        ...
        if (file || !file.type.startsWith("image/")) {
            toast.error("We cannot process video at the moment")
        }
        ...
        <div className='h-full overflow-auto relative backdrop-blur-lg'>
            ...
            <div className='flex flex-col h-[calc(100%-120px)] overflow-y-auto p-3 pb-6 scrollbar-hide'>
                {messages.map((message, index) => (
                    <div key={index} className={`flex items-end gap-2 justify-end ${message.senderId !== authUser._id && 'flex-row-reverse'}`}>{message.image ? (
            ...
            <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
                <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                    ...
                    <input onChange={handleSendingImage} type="file" name="" id="image" accept='image/png, image/jpeg' hidden />
